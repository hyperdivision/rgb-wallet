...
function transferAsset (wallet, request, changeAddress) {
  ...
  for (const asset of Object.keys(assetRequest)) {
    const assetInfo = assets.find(info => info.asset === asset)
    assert(assetInfo.amount >= assetRequest[asset],
      'Insufficient assets')
  }
  ...
  proofInputs.map((proof) => {
    const seal = proof.seals.find((seal) => seal.outpoint === `${proof.tx}:${proof.vout}`)
  ...
  for (const [asset, amount] of Object.entries(amounts)) {
    ...
    for (const transfer of request) {
      if (transfer[asset] === undefined) continue
      changeAmount -= transfer[asset]
    }

    const changeOutput = {}
    changeOutput.address = changeAddress.pop()
    changeOutput[asset] = changeAmount
...
function sortByUTXO (assets, transfers) {
  ...
  for (const ownedAsset of assets) {
    ...
    const label = `${ownedAsset.tx}:${ownedAsset.vout}`
