...
function receiveAsset (inputs, utxos, opts) {
  ...
  transferProof.pubkey = inputs.originalPubKey
  let tweakIndex = null

  if (transferProof.pubkey) {
    const PK = Buffer.from(transferProof.pubkey, 'hex')
    ...
    rpcInfo.outputs = outputs.map(output => {
      if (Object.keys(output)[0] === commitmentAddress) {
        ...
        console.log(PK)
        return buildTx.payToContract(Object.values(output)[0], serializedProof, PK, 'proof', 'testnet')
      } else return output
    })
  } else {
    // TODO op_return
