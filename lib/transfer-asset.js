...
function transferAsset (wallet, request, changeAddress) {
  ...
  proofInputs.map((proof) => {
    const seal = proof.seals.find((seal) => seal.outpoint === `${proof.txid}:${proof.vout}`)
    amounts[seal.ticker] = amounts[seal.ticker] || 0
    amounts[seal.ticker] += seal.amount
  })

  for (let [asset, amount] of Object.entries(amounts)) {
    let changeAmount = assets[asset].amount
...
function sortRequest (request) {
  const sortedRequest = {}
  request.map(entry => {
    sortedRequest[entry.asset] = entry.amount
  })
