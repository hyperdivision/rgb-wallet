...
//   // strategy -> for each root: walk down to leading proof
//   // 1) check which seals are spent
//   // 2) use findSpendingTx to find next proof along and repeat.
...
//   // check proof etc
//   // use findSpendingTx to repeat process
// }

// need fucntion to determine full seal spending root with assets assigned
// ie. proofHistory -> formatted history of seals:assets + spendingTxs for each proof
...
async function checkProof (proofHistory, finalProof, client) {
  ...
  for (const root of roots) {
    ...
    console.log(rootProof)

    const asset = rootProof.fields.ticker
    assets[asset] = assets[asset] || []

    await walkProofs(rootProof.seals, asset, level).then(() => {
      /////////////////// ////////
  ...
  async function walkProofs (seals, asset, level) {
    ...
    for (const seal of seals) {
      ...
      console.log(proofIndex, 'proofInedx')
    ...
    // console.log(nextProofs, '_______________________________')
...
async function findSpendingTx (client, spent) {
  ...
  for (let i = blockHeight - 1; i > 0; i--) {
    ...
    for (const txid of blockInfo.tx) {
      ...
      if (tx.vin.findIndex(vin => vin.txid === spent.txid) !== -1) {
...
checkProof(proofHistory.slice(), proofHistory.slice().pop(), w.client)
