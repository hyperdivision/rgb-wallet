...
async function transferAsset (wallet, request) {
  ...
  const io = txBuilder(selectedInputs, request, transferProof)
  const rawTx = await client.createRawTransaction(io.inputs, io.outputs)

  //  5. add transacaction id to transfer proof
  client.decodeRawTransaction(rawTx).then((tx) => {
  ...
  client.signRawTransactionWithWallet(rawTx)
    .then((tx) => client.sendRawTransaction(tx.hex))
  ...
  // make transfer tx
  function txBuilder (selectedInputs, request, proof) {
    ...
    if (proof.originalPK) {
      ...
      rpcOutputs.map(output => {
        if (Object.keys(output)[0] === commitmentAddress) {
          output = keys.tweakOutput(output, PK, proofHash, 'proof')
        }
        return output
      })
    }
